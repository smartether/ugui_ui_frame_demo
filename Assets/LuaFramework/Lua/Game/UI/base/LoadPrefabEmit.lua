local LoadPrefabEmit = LuaClass()

local util = {}
util.pack = table.pack or function(...) return { n = select('#', ...), ... } end
util.unpack = table.unpack or unpack

function LoadPrefabEmit:ctor(type, module, view, parent, args, onfinished)
    self._module = module
    self._type = type
    self._view = view
    self._parent = parent
...
function LoadPrefabEmit:Init(type, module, view, parent, args, onfinished)
    self._module = module
    self._type = type
    self._view = view
    self._parent = parent
...
function LoadPrefabEmit:Emit(go)
